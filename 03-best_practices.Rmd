...
Code files are stored in the top level of the `project` directory (e.g. `01-project.Rmd`). Also stored in the top level directory is a `README.md` file that contains a description of the project and a brief summary of for each file and folder in the project.  
...
It is a good idea to track package versions used in your code. That way, if anything changes in the package, you will know what version is needed to make your code work. There are a few different ways to track package versions in R Projects. One is [{packrat}](http://rstudio.github.io/packrat/), which when it works provides a snapshot of all your package versions for a project. However, it is difficult to use and can be buggy. Another option still earlyi in development is [{renv}](https://github.com/rstudio/renv), which like {packrat} uses private package libraries to track package versions. {renv} is by developed by RStudio and is therefore likely to provide code base with ongoing maintenance.  

# 5 Use functions
If you're copy pasting code with few changes, write a function instead. [Nice R Code](https://nicercode.github.io/guides/functions/) has a good introduction to writing functions for those new to the idea. For a more in-depth introduction to functions, see Hadley Wickham's chapters on functional programming in the [R for Data Science](https://r4ds.had.co.nz/functions.html) book, or see a more in depth discussion in his [Advanced R](https://adv-r.hadley.nz/functionals.html) book. It is generally better to [choose vectorization over loops](https://nicercode.github.io/guides/repeating-things/) as functions require less text and can run more quickly.  
